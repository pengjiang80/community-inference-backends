...
To simplify usage in GPUStack, the image is repackaged with:

* A simplified entrypoint script
* Default user changed to `root`
* Flexible argument passing

### Dockerfile

```dockerfile
FROM swr.cn-south-1.myhuaweicloud.com/ascendhub/mis-tei:7.3.0-800I-A2-aarch64

USER 0:0

COPY start.sh /home/HwHiAiUser/start.sh
```
...
export LD_PRELOAD=$(ls /usr/local/lib/python3.11/site-packages/scikit_learn.libs/libgomp-* 2>/dev/null):$LD_PRELOAD
...
exec text-embeddings-router --hostname 0.0.0.0 "$@"
```

### Notes

* Removes unnecessary logic from the original script
* Supports arbitrary parameter passing via `"$@"`
* Ensures proper Ascend runtime environment initialization
...
Additional parameters not explicitly defined in the backend configuration can still be passed through directly.

To view all supported options:

```bash
docker run --rm -it gpustackcommunity/mis-tei:7.3.0-800I-A2-aarch64 --help
```
